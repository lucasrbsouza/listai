...
class PptxFormatter implements ExportFormatter {
  ...
  Future<File> export(ShoppingList list) async {
    ...
    outputArchive.addFile(
      ArchiveFile(
        '[Content_Types].xml',
        contentTypesStr.codeUnits.length,
        contentTypesStr.codeUnits,
      ),
    );
    ...
    const relsStr =
        ...
        '</Relationships>';
    outputArchive.addFile(
      ArchiveFile('_rels/.rels', relsStr.codeUnits.length, relsStr.codeUnits),
    );

    // 4. ppt/presentation.xml
    final presentationSb = StringBuffer();
    presentationSb.writeln(
      '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>',
    );
    presentationSb.writeln(
    ...
    outputArchive.addFile(
      ArchiveFile(
        ...
        presentationStr.codeUnits.length,
        presentationStr.codeUnits,
      ),
    );

    // 5. ppt/_rels/presentation.xml.rels
    final presentationRelsSb = StringBuffer();
    presentationRelsSb.writeln(
      '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>',
    );
    presentationRelsSb.writeln(
      '<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">',
    ...
    outputArchive.addFile(
      ArchiveFile(
        ...
        presentationRelsStr.codeUnits.length,
        presentationRelsStr.codeUnits,
      ),
    );

    // 6. ppt/slides/slide1.xml (Capa)
    final marketStr = list.marketName != null && list.marketName!.isNotEmpty
    ...
    final outputFile = File(
      '${tempDir.path}/export_${list.id}_${DateTime.now().millisecondsSinceEpoch}.pptx',
    );
