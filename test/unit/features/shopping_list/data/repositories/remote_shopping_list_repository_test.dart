...
class MockPostgrestFilterBuilder extends Mock
  ...
  @override
  MockPostgrestFilterBuilder eq(final String column, final Object value) =>
      this;

  @override
  MockPostgrestFilterBuilder order(
    final String column, {
    final bool ascending = true,
    final bool nullsFirst = false,
    final String? referencedTable,
  }) => this;

  @override
  MockPostgrestFilterBuilder limit(
    final int count, {
    final String? referencedTable,
  }) => this;
  ...
  @override
  Future<T> then<T>(
    final FutureOr<T> Function(List<Map<String, dynamic>>) onValue, {
    ...
    final data = mockDataQueue.isNotEmpty
        ? mockDataQueue.removeAt(0)
        : <Map<String, dynamic>>[];
    return Future.value(
...
class MockRpcFilterBuilder extends Mock
  ...
  @override
  Future<T> then<T>(
    final FutureOr<T> Function(dynamic) onValue, {
    final Function? onError,
  }) {
    if (forceError != null) {
...
void main() {
      ...
      final itemData = [
        {
          ...
          'photo_captured_at': list.items.first.photoCapturedAt
              ?.toIso8601String(),
  ...
  group('saveCurrentList', () {
    test('inserts list and items in a batch', () async {
      final list = createTestList();
      mockFilterBuilder.mockDataQueue = [
        <Map<String, dynamic>>[],
        <Map<String, dynamic>>[],
      ];
      ...
      verify(
        () => mockSupabaseClient.from('shopping_items'),
      ).called(2); // Delete & Insert
    });
