...
void main() {
  ...
  group('SyncManager - Offline to Online Upload', () {
    test('uploads pending lists to remote when online', () async {
      ...
      when(() => mockLocalRepository.getPendingUploads()).thenAnswer((_) async => [list]);
      when(() => mockRemoteRepository.getListById('list-1')).thenThrow(NotFoundFailure('Not found'));
      when(() => mockRemoteRepository.saveCurrentList(list)).thenAnswer((_) async {});
      when(() => mockLocalRepository.updateSyncStatus('list-1', 'synced')).thenAnswer((_) async {});

      await syncManager.sync();

      verify(() => mockRemoteRepository.saveCurrentList(list)).called(1);
      verify(() => mockLocalRepository.updateSyncStatus('list-1', 'synced')).called(1);
    });
  ...
  group('SyncManager - Conflict Resolution', () {
    test('resolves conflict with Last-Write-Wins (Local is Newer)', () async {
      ...
      final localList = createTestList(id: 'list-1', updatedAt: now.add(const Duration(minutes: 5)));
      final remoteList = createTestList(id: 'list-1', updatedAt: now);

      when(() => mockLocalRepository.getPendingUploads()).thenAnswer((_) async => [localList]);
      when(() => mockRemoteRepository.getListById('list-1')).thenAnswer((_) async => remoteList);
      when(() => mockRemoteRepository.saveCurrentList(localList)).thenAnswer((_) async {});
      when(() => mockLocalRepository.updateSyncStatus('list-1', 'synced')).thenAnswer((_) async {});
    ...
    test('resolves conflict with Last-Write-Wins (Remote is Newer) - Marks local as synced and overwrites local', () async {
      final now = DateTime.now();
      final localList = createTestList(id: 'list-1', updatedAt: now);
      final remoteList = createTestList(id: 'list-1', updatedAt: now.add(const Duration(minutes: 5)));

      when(() => mockLocalRepository.getPendingUploads()).thenAnswer((_) async => [localList]);
      when(() => mockRemoteRepository.getListById('list-1')).thenAnswer((_) async => remoteList);
      when(() => mockLocalRepository.saveCurrentList(remoteList)).thenAnswer((_) async {});
      when(() => mockLocalRepository.updateSyncStatus('list-1', 'synced')).thenAnswer((_) async {});
  ...
  group('SyncManager - Local Data Migration', () {
    test('migrates local offline data to cloud', () async {
      final list = createTestList(id: 'list-offline', updatedAt: DateTime.now()).copyWith(userId: null);

      when(() => mockLocalRepository.getPendingUploads()).thenAnswer((_) async => [list]);
      when(() => mockRemoteRepository.saveCurrentList(any())).thenAnswer((_) async {});
      when(() => mockLocalRepository.saveCurrentList(any())).thenAnswer((_) async {});
      when(() => mockLocalRepository.updateSyncStatus(any(), any())).thenAnswer((_) async {});
